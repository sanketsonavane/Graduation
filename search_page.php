...
<?php 
	$s=$_GET['search'];
	$str="SELECT * FROM question, user WHERE question.user_id=user.user_id ";
	...
	if ($no_rows > 0)
	{
		...
		{
			$sql="SELECT count(*) as counter from answer where question_id=$row[question_id]";
			$rowsc=ExecuteQuery($sql);
			$rowc = mysql_fetch_array($rowsc);

			$count = $rowc['counter'];
			
			if(stripos($row["heading"], $s)!==false)
			{
